...
A working prototype for **AI-Powered Financial Inclusion: Dynamic Risk Assessment for Underserved Segments**. It scores a borrower who has no credit history from fourteen facts about the household's income, costs, loans and payment record and the loan it asks for, shows how much each factor contributed, and has an AI write a short plain-language explanation, so a loan officer can explain the decision.
...
2. They enter the household's facts and the loan requested, or pick one of five example borrowers.
3. The API returns a score out of 100, a risk band (Low, Medium or High), and the points each of ten factors added.
...
5. Every assessment is saved to a PostgreSQL database and shown in a history table, with a fairness check that compares outcomes across urban, semi-urban and rural households.
...
| Tests | pytest, 123 tests | `backend/test_model.py`, `backend/test_auth.py`, `backend/test_explain.py`, `backend/test_fairness.py` |

How a request flows:

1. The browser sends the username and password to `POST /login` and receives a token that lasts 12 hours.
2. The browser sends the borrower's facts and the loan requested to `POST /assess-risk` with the token.
3. The API checks the token, validates the inputs, computes the score, and saves the assessment to Postgres, with the type of area kept apart for the fairness audit. It then asks the AI for a short explanation, sending only the numbers behind the score, and returns the result with its breakdown and the explanation.
4. `GET /history` returns recent assessments, again only with a valid token.

## How the score works

A lender asks three questions about a household. Can it afford the loan? Will it pay? What happens if something goes wrong? The scorecard turns the facts into ten factors that answer those questions, then adds them up with fixed weights. The reasoning behind every choice, what was deliberately left out, and where each number would come from in a real system, is in **[DESIGN.md](DESIGN.md)**.

| Question | Factor | Measured as | Weight |
|---|---|---|---|
| Can they afford it? | Loan burden | Existing loan payments plus the new installment, as a share of household income. Full points at 30% or less, none at 50% | 15% |
| | Disposable income | Income left after essential costs and all loan payments, as a share of income. 20% or more earns full points | 15% |
| | Income stability | The lowest month's income as a share of the average month over 6 months | 15% |
| | Income per person | Household income divided by household size. ₹12,500 a person earns full points | 5% |
| Will they pay? | Bills paid on time | Utility, rent and phone bills paid by the due date, out of the last 12 | 15% |
| | Failed payments | Bounced automatic payments in 6 months: none is full points, then 60, 30 and 0 for three or more | 8% |
| | Active loans | Loans being repaid now: none is full points, then 85, 60, 30 and 0 for four or more | 7% |
...
- **The loan asked for matters.** The app takes the loan amount, tenure and interest rate and works out the monthly installment with the standard EMI formula, so the same household can be fine for a small loan and refused for a large one.
- **Essential costs have a floor.** People understate what they spend, so the scorecard uses the declared essential costs or ₹3,000 per household member, whichever is higher.
- **Risk bands:** 70 and above is Low Risk, 40 to below 70 is Medium Risk, below 40 is High Risk.
- **Policy rules.** Three facts force the band to High Risk whatever the score is, because a lender would not lend in these cases: no income in the last 6 months; loan payments above 50% of household income, which is the RBI's limit for low-income households; and income that cannot cover essential costs plus loan payments. The app shows the rule when it applies.
- **The type of area is never scored.** Urban, semi-urban or rural is saved with each assessment only so that outcomes can be compared across areas, and a test proves it cannot change a score. Gender, caste, religion and marital status are not collected at all.
- **A vehicle with a running loan** is counted under existing loan payments, because a financed vehicle is a debt and not only an asset.

The five example households in the app score 89.48 (Low), 56.32 (Medium), 33.58 (High), 40 with no income, which the no-income rule sets to High Risk, and 59.48 for the steady household asking for ₹3,00,000, which the loan-burden and living-costs rules set to High Risk.
...
| GET | `/fairness-report` | Yes | Outcomes grouped by type of area, for auditing |

`/assess-risk` returns `final_score`, `risk_band`, `breakdown` (points per factor), `details` (the fact behind each factor), `overrides` (any policy rule that applied), `loan_emi` (the monthly installment of the requested loan), `residual_income` (what is left after essentials and loan payments) and `ai_explanation`. The last is `null` when the AI layer is off or unavailable.

Inputs to `/assess-risk`, all required. Money is in rupees a month unless stated.

| Input | Meaning | Allowed |
|---|---|---|
| `monthly_income` | Household income, averaged over the last 6 months | 0 or more |
| `lowest_month_income` | Lowest month's income in the same period | 0 up to `monthly_income` |
| `household_size` | People sharing the household's income | 1 to 20 |
| `household_expenses` | Essential costs: food, rent, school, medical, transport | 0 or more |
| `existing_loan_payments` | Installments and card dues already being paid | 0 or more |
| `active_loans` | Loans being repaid now, from any lender | 0 to 20 |
| `loan_amount` | Amount of the loan requested, in rupees | 0 or more |
| `tenure_months` | Months to repay | 1 to 120 |
| `annual_rate_percent` | Interest rate a year | 0 to 60 |
...
| `avg_balance` | Average month-end balance, last 6 months, in rupees | 0 or more |
...
| `area_type` | `urban`, `semi_urban` or `rural`. Saved for the fairness audit, never scored | one of these |
...
| Git, README, setup instructions, architecture diagram, tests | GitHub, this README, DESIGN.md, the diagram above, 123 tests | Built |
...
**Prototype limits**
- The facts are entered by hand, and nothing checks that they are true. A real system would derive them from consented data such as bank and UPI transactions, utility and telecom records, and each one has a defined way of being measured, as in the table above.
- The weights and thresholds are hand-set judgement. The three policy rules are examples of the hard rules a lender applies before any score.
- The fairness table is built from a handful of demo assessments. It shows the mechanism, not evidence.
...
- Test every factor for bias against groups the lender must treat fairly. Assets, time in work and income per person need particular care, so each carries only 5% of the score.
...
```
DESIGN.md             what the scorecard measures and why
backend/
  ...
  test_fairness.py    tests that the area type never changes a score
